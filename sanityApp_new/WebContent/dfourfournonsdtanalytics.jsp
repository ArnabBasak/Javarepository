...
<title>D2.4.44 Non Rollout Analytics</title>
...
<script type="text/javascript" src="https://www.google.com/jsapi"></script>
    <script type="text/javascript">
      ...
      function drawChart() {
        ...
        ['Failed', 'Success'],
   		['EOD Failures',80 ],//#cc0000
   	 	['Replication Failures',17], //#ff0066
   	 	['Upgrade Failures', 95], //#993366
        ['Successful EOD',697 ],// #66ff33
        ['Successful Replication', 760], //#008000
        ['Successful Upgrades',682] //#00cc66
...
PreparedStatement failedstores = connection.prepareStatement("select count(*) from deployment where dep_duration = 0 && version like 'D2.4.44'");
...
PreparedStatement rolloutdate = connection.prepareStatement("select rolloutdate from releasedates where rollout like 'D2.4.44'");
...
PreparedStatement successfulstore = connection.prepareStatement("select count(*) from deployment where Dep_Duration != 0 && version like 'D2.4.44'");
...
PreparedStatement totalstores = connection.prepareStatement("select count(*) from deployment where version like 'D2.4.44'");
...
PreparedStatement eodfail = connection.prepareStatement("select count(*) from deployment where eod_duration = 0 && version like 'D2.4.44'");
...
PreparedStatement replicationfailed = connection.prepareStatement("select count(*) from deployment where replication_duration = 0 && version like 'D2.4.44'");
...
PreparedStatement depfailed = connection.prepareStatement("select count(*) from deployment where eod_duration != 0 && replication_duration != 0 && dep_duration = 0 && version like 'D2.4.44'");
...
PreparedStatement maxeod = connection.prepareStatement("select name,eod_duration from deployment where eod_duration IN (select max(eod_duration) from deployment where version like 'D2.4.44');");
...
PreparedStatement mineod = connection.prepareStatement("select name,eod_duration from deployment where eod_duration IN (select min(eod_duration) from deployment where version like 'D2.4.44' && eod_duration!=0);");
...
PreparedStatement maxreplication = connection.prepareStatement("select name,replication_duration from deployment where replication_duration IN (select max(replication_duration) from deployment where version like 'D2.4.44');");
...
PreparedStatement minreplication = connection.prepareStatement("select name,replication_duration from deployment where replication_duration IN (select min(replication_duration) from deployment where version like 'D2.4.44' && replication_duration!=0) limit 1");
...
PreparedStatement maxdepduration = connection.prepareStatement("select name,dep_duration from deployment where dep_duration IN (select max(dep_duration) from deployment where version like 'D2.4.44')");
...
PreparedStatement mindepduration = connection.prepareStatement("select name,dep_duration from deployment where dep_duration IN (select min(dep_duration) from deployment where version like 'D2.4.44' && dep_duration !=0) limit 1");
...
PreparedStatement eodsuccess = connection.prepareStatement("select count(*) from deployment where eod_duration !=0 && version = 'D2.4.44'");
...
PreparedStatement depsuccess = connection.prepareStatement("select count(*) from deployment where dep_duration !=0 && version = 'D2.4.44' && eod_duration !=0 && replication_duration !=0;");
...
PreparedStatement repsuccess = connection.prepareStatement("select count(*) from deployment where replication_duration !=0 && version = 'D2.4.44'");
    ...
    <div class="site-wrapper active" id="target">
      <div class="site-wrapper_left-col">
        ...
        <div class="left-nav">
		  ...
		  <a href="dfourfournonsdtanalytics.jsp" class="active"><i class="fa fa-pie-chart"></i>D2.4.44</a>
      ...
      <div class="row">
          ...
          <div class="user-item">
            <div class="user-item_info">
              <p class="name"><h3>D2.4.44 Rollout information</h3><br><p class="name" style="color:#990033;">Rollout Date&nbsp;&nbsp;<%out.println(rolloutdatecount);%></p>
