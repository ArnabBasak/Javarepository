...
<title>D2.4.42 Non Rollout Analytics</title>
      ...
      function drawChart() {
        ...
        ['Failed', 'Success'],
   		['EOD Failures',108 ],//#cc0000
   	 	['Replication Failures',45], //#ff0066
   	    ['Upgrade Failures', 137], //#993366
        ['Successful EOD',669 ],// #66ff33
        ['Successful Replication', 732], //#008000
        ['Successful Upgrades',640] //00cc66
...
PreparedStatement failedstores = connection.prepareStatement("select count(*) from deployment where dep_duration = 0 && version like 'D2.4.42'");
...
PreparedStatement rolloutdate = connection.prepareStatement("select rolloutdate from releasedates where rollout like 'D2.4.42'");
...
PreparedStatement successfulstore = connection.prepareStatement("select count(*) from deployment where Dep_Duration != 0 && version like 'D2.4.42'");
...
PreparedStatement totalstores = connection.prepareStatement("select count(*) from deployment where version like 'D2.4.42'");
...
PreparedStatement eodfail = connection.prepareStatement("select count(*) from deployment where eod_duration = 0 && version like 'D2.4.42'");
...
PreparedStatement replicationfailed = connection.prepareStatement("select count(*) from deployment where replication_duration = 0 && version like 'D2.4.42'");
...
PreparedStatement depfailed = connection.prepareStatement("select count(*) from deployment where eod_duration != 0 && replication_duration != 0 && dep_duration = 0 && version like 'D2.4.42'");
...
PreparedStatement maxeod = connection.prepareStatement("select name,eod_duration from deployment where eod_duration IN (select max(eod_duration) from deployment where version like 'D2.4.42');");
...
PreparedStatement mineod = connection.prepareStatement("select name,eod_duration from deployment where eod_duration IN (select min(eod_duration) from deployment where version like 'D2.4.42' && eod_duration!=0);");
...
PreparedStatement maxreplication = connection.prepareStatement("select name,replication_duration from deployment where replication_duration IN (select max(replication_duration) from deployment where version like 'D2.4.42');");
...
PreparedStatement minreplication = connection.prepareStatement("select name,replication_duration from deployment where replication_duration IN (select min(replication_duration) from deployment where version like 'D2.4.42' && replication_duration!=0) limit 1");
...
PreparedStatement maxdepduration = connection.prepareStatement("select name,dep_duration from deployment where dep_duration IN (select max(dep_duration) from deployment where version like 'D2.4.42')");
...
PreparedStatement mindepduration = connection.prepareStatement("select name,dep_duration from deployment where dep_duration IN (select min(dep_duration) from deployment where version like 'D2.4.42' && dep_duration !=0) limit 1");
...
PreparedStatement eodsuccess = connection.prepareStatement("select count(*) from deployment where eod_duration !=0 && version = 'D2.4.42'");
...
PreparedStatement depsuccess = connection.prepareStatement("select count(*) from deployment where dep_duration !=0 && version = 'D2.4.42' && eod_duration !=0 && replication_duration !=0;");
...
PreparedStatement repsuccess = connection.prepareStatement("select count(*) from deployment where replication_duration !=0 && version = 'D2.4.42'");
    ...
    <div class="site-wrapper active" id="target">
      <div class="site-wrapper_left-col">
        ...
        <div class="left-nav">
		  ...
		  <a href="dfourtwononsdtanalytics.jsp" class="active"><i class="fa fa-pie-chart"></i>D2.4.42</a>
      ...
      <div class="row">
          ...
          <div class="user-item">
            <div class="user-item_info">
              <p class="name"><h3>D2.4.42 Rollout information</h3><br><p class="name" style="color:#990033;">Rollout Date&nbsp;&nbsp;<%out.println(rolloutdatecount);%></p>
